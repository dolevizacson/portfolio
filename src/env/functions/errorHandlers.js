...
export const generalErrorHandler = (error, errorMessagesObject) => {
  if (error.response) {
    switch (error.response.status) {
      case httpStatus.UNAUTHORIZED:
        throw new UnAuthorizedError(errorMessagesObject.unAuthorizedError);
      default:
        throw new InternalServerError(errorMessagesObject.internalServerError);
    }
  } else if (error.request) {
    throw new NoResponseError(errorMessagesObject.noResponseError);
  } else {
    throw new BadRequestObjectError(errorMessagesObject.badRequestObjectError);
  }
};

export const localStorageErrorHandler = (error, message) => {
  if (message) {
